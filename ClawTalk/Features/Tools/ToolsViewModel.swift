...
final class ToolsViewModel {
    ...
    private func log(_ message: String) {
        ClawTalkLogStore.shared.append("ToolsViewModel: \(message)")
    }

    private func presentError(_ message: String, log diagnostic: String? = nil) {
    ...
    private func messageForToolFailure(_ error: Error, feature: String) -> String {
        if let openClawError = error as? OpenClawError {
            switch openClawError {
            case .toolError(let message):
                return message
            case .toolNotFound:
                return "\(feature) 依赖扩展接口，但当前 IronClaw 未启用对应工具。聊天主链路仍可使用。"
            default:
                return openClawError.localizedDescription
            }
        }
    ...
    func searchMemory() async {
        ...
        do {
            ...
            log("memory_search 成功 count=\(memoryResults.count)")
        } catch {
            presentError(messageForToolFailure(error, feature: "记忆搜索"), log: "memory_search 失败 error=\(error.localizedDescription)")
        }
    ...
    func getMemoryFile(path: String, from: Int? = nil, lines: Int? = nil) async {
        ...
        do {
            ...
            log("memory_get 成功 path=\(path)")
        } catch {
            presentError(messageForToolFailure(error, feature: "记忆文件读取"), log: "memory_get 失败 path=\(path) error=\(error.localizedDescription)")
        }
    ...
    func listAgents() async {
        ...
        do {
            ...
            log("agents_list 成功 count=\(agents.count)")
        } catch {
            presentError(messageForToolFailure(error, feature: "代理列表"), log: "agents_list 失败 error=\(error.localizedDescription)")
        }
    ...
    func listSessions() async {
        ...
        do {
            ...
            log("sessions_list 成功 count=\(sessions.count)")
        } catch {
            presentError(messageForToolFailure(error, feature: "会话列表"), log: "sessions_list 失败 error=\(error.localizedDescription)")
        }
    ...
    func getSessionStatus(sessionKey: String? = nil) async {
        ...
        do {
            ...
            sessionStatus = wrapper.details?.statusText
                ...
                ?? "暂无状态信息"
            log("session_status 成功 session=\(sessionKey ?? "none")")
        } catch {
            presentError(messageForToolFailure(error, feature: "会话状态"), log: "session_status 失败 session=\(sessionKey ?? "none") error=\(error.localizedDescription)")
        }
    ...
    func getSessionHistory(sessionKey: String, limit: Int = 20) async {
        ...
        do {
            ...
            log("sessions_history 成功 session=\(sessionKey)")
        } catch {
            presentError(messageForToolFailure(error, feature: "会话历史"), log: "sessions_history 失败 session=\(sessionKey) error=\(error.localizedDescription)")
        }
    ...
    func getBrowserStatus() async {
        ...
        do {
            ...
            log("browser/status 成功")
        } catch {
            presentError(messageForToolFailure(error, feature: "浏览器状态"), log: "browser/status 失败 error=\(error.localizedDescription)")
        }
    ...
    func takeBrowserScreenshot() async {
        ...
        do {
            ...
            log("browser/screenshot 成功")
        } catch {
            presentError(messageForToolFailure(error, feature: "浏览器截图"), log: "browser/screenshot 失败 error=\(error.localizedDescription)")
        }
    ...
    func getBrowserTabs() async {
        ...
        do {
            ...
            presentError(messageForToolFailure(error, feature: "浏览器标签页"), log: "browser/tabs 失败 error=\(error.localizedDescription)")
        }
    ...
    func loadModels() async {
        ...
        guard settings.isConfigured else {
            presentError("请先在设置中配置 IronClaw 地址与 Token。")
            return
        }
        ...
        do {
            ...
            log("/v1/models 成功 count=\(availableModels.count)")
        } catch {
            presentError(error.localizedDescription, log: "/v1/models 失败 error=\(error.localizedDescription)")
        }
    }
}
    // Memory
